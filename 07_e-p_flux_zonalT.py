    ...
    for ayear in range(startYear,endYear+1):
        for aday in range(1,calendar.monthrange(ayear,amonth)[1]+1):
            ...
            if ayear == startYear and aday ==1:
                ...
                zonalU_3d = np.append(zonalU_3d,zonalT[np.newaxis],axis=0)
...
def draw():
    ...
    for y in range(2):
        if y == 0:
            Fy, Fz, nablaF,zonalT = monthYearMean(meanstart,meanend,month)
            title = f'{meanstart}to{meanend} mean'
        else:
            Fy, Fz, nablaF,zonalT = monthMean(year,month)
            title = str(year)
        # cont = axes[axnum].contour(X,Y,zonalhgt,colors='black')
        # cont = axes[y].contour(X,Y,zonalT,levels=np.linspace(-100,100,25),linewidths=0.75, cmap='plasma_r')
        cont = axes[y].contour(X,Y,zonalT,levels=np.arange(100,400,5),linewidths=0.75, colors='black',alpha=0.65)
    ...
    plt.savefig(f'D:/picture/study/MLS/monthYearMean/zonalT/month{month}Mean_{meanstart}to{meanend}and{year}_E-Pflux_zonalT.png')
    plt.show()
