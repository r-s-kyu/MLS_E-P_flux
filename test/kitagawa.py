...
for year in range(syear,fyear+1) :
	...
	f = np.broadcast_to(cori,(day,nz,ny))
	f4 = np.broadcast_to(f.T,(nx,ny,nz,day)).T
	cos = np.broadcast_to(cosine,(day,nz,ny))
	cos4 = np.broadcast_to(cos.T,(nx,ny,nz,day)).T
	# print('f:',f.shape, f4.shape)
	# print('cos:',cos.shape, cos4.shape)

	############################	load file
	data_gph = f'/Users/kitagawa/desktop/data/MLS/grid_data/GPH/MLS_GPH_{year}_Moving3days.npz'
	data_T = f'/Users/kitagawa/desktop/data/MLS/grid_data/Temp/MLS_Temp_{year}_Moving3days.npz'

	f_gph = np.load(data_gph)
	f_temp = np.load(data_T)

	gph = f_gph['data']				# GeoPotentialHeight
	temp = f_temp['data']			# Temperature
