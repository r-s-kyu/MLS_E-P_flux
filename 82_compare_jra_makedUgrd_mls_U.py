...
ugrdJRA = f'D:/data/JRA55/zonal_deviation/{name}/{year}/{year}d{str(dc).zfill(3)}_{name}_{kind}.npy'
...
fig.suptitle(f'{month}/{day}/{year} prs={pressure} deviation of ugrd ',fontsize=15)
axpos = axes[0].get_position()
cbar_ax = fig.add_axes([0.91, axpos.y0, 0.02, axpos.height])
fig.colorbar(contf,cax=cbar_ax)
path = f'D:/picture/study/JRA55/compare'
if not os.path.exists(path):
    os.mkdir(path)
if not os.path.exists(path+f'/ugrd'):
    os.mkdir(path+f'/ugrd')
if not os.path.exists(path+f'/ugrd'+f'/pressure'):
    os.mkdir(path+f'/ugrd'+f'/pressure')
plt.savefig(f'D:/picture/study/JRA55/compare/ugrd/pressure/d{year}{str(month).zfill(2)+str(day).zfill(2)}_prs={pressure}_devugrd_in_JRA_MLS.png')
# plt.show()
