...
fig.suptitle(f'{month}/{day}/{year} prs={pressure} deviation of U ',fontsize=15)
...
if not os.path.exists(path+f'/U'):
    os.mkdir(path+f'/U')
if not os.path.exists(path+f'/U'+f'/pressure'):
    os.mkdir(path+f'/U'+f'/pressure')
plt.savefig(f'D:/picture/study/MLS/compare/U/pressure/d{year}{str(month).zfill(2)+str(day).zfill(2)}_prs={pressure}_devugrd_in_JRA_MLS.png')
plt.show()
