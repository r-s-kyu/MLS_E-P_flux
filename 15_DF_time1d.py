...
def monthYearMean(startYear,endYear):
    ...
    for amonth in range(1,13):
        for aday in range(1,calendar.monthrange(notUru,amonth)[1]+1): #うるう年でなければ何でもよい
            ...
            nf_2d = np.zeros((meanyears,latIndex[1]-latIndex[0]+1),np.float32)
            u_2d = np.zeros((meanyears,latIndex[1]-latIndex[0]+1),np.float32)
            for ayear in range(startYear,endYear+1):
                ...
                for ind in range(latIndex[0],latIndex[1]+1):
                    nf_2d[ayear-startYear,ind-latIndex[0]] = nF[prsInd,ind]
                    u_2d[ayear-startYear,ind-latIndex[0]] = zonalU[prsInd,ind]
            yearMeanDF[dnum-1] = np.nansum(nf_2d*w_lat)/np.sum(w_lat[~np.isnan(nf_2d)])
            yearMeanU[dnum-1] = np.nansum(u_2d*w_lat)/np.sum(w_lat[~np.isnan(u_2d)])
            print(f'{amonth}/{aday}')
    print(f'finish YearMeanFz!')
    return yearMeanDF, yearMeanU
...
meanDF, meanU = monthYearMean(meanstart, meanend)
Df,U = monthMean(year)
print("start yMean")
...
axes[0].set_ylabel('Fz',fontsize=15)
