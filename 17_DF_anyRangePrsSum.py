...
prslist = [1.0000000e+01, 6.8129206e-01]
...
with open(prsfile,'rb') as r:
    pcord = np.load(r)
prsIndlist = np.array([],dtype=np.int32)
for meanprs in prslist:
    try:
        prsInd = np.where(pcord==meanprs)[0][0]
        prsIndlist = np.append(prsIndlist,prsInd)
    except:
        print('Error')
...
def monthYearMean(startYear,endYear):
    # notUru = 2021
    # w_lat = np.broadcast_to(cosphi,(prsIndlist[1]-prsIndlist[0]+1,len(cosphi)))
    w_lat = cosphi
    ...
    for amonth in range(1,13):
        for aday in range(1,calendar.monthrange(notUru,amonth)[1]+1): #うるう年でなければ何でもよい
            ...
            for ayear in range(startYear,endYear+1):
                ...
                nf_lat = nF[prsIndlist[0]:prsIndlist[1]+1,latIndex[0]:latIndex[1]+1]
                u_lat = zonalU[prsIndlist[0]:prsIndlist[1]+1,latIndex[0]:latIndex[1]+1]
                nf_lat = np.nansum(nf_lat,axis=0)
                u_lat = np.nanmean(u_lat,axis=0)
                nf_year[ayear-startYear] = np.nansum(nf_lat*w_lat)/np.sum(w_lat[~np.isnan(nf_lat)])
                u_year[ayear-startYear] = np.nansum(u_lat*w_lat)/np.sum(w_lat[~np.isnan(u_lat)])
...
def monthMean(ayear):
    ...
    for amonth in range(1,13):
        for aday in range(1,calendar.monthrange(notUru,amonth)[1]+1): #うるう年でなければ何でもよい
            ...
            nf_lat = np.nansum(nf_lat,axis=0)
            u_lat = np.nanmean(u_lat,axis=0)
            oneYearMeanDF[dnum-1] = np.nansum(nf_lat*w_lat)/np.sum(w_lat[~np.isnan(nf_lat)])
            oneYearMeanU[dnum-1] = np.nansum(u_lat*w_lat)/np.sum(w_lat[~np.isnan(u_lat)])
...
axes[0].set_title(f'prsSum{prslist[0]:.2f}-{prslist[1]:.2f}hPa  latMean{meanlatrange[0]}to{meanlatrange[1]}',fontsize=15)
axes[0].grid(True)
axes[1].grid(True)
plt.savefig(f'D:/picture/study/MLS/e-p_flux/DF_U/anyPrsMean/prs{meanprs:.07f}_lat{meanlatrange[0]}to{meanlatrange[1]}Mean_E-Pflux_DF.png')
plt.show()
