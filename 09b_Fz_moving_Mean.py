...
def monthYearMean(startYear,endYear):
    ...
    for amonth in range(1,13):
        for aday in range(1,calendar.monthrange(notUru,amonth)[1]+1): #うるう年でなければ何でもよい
            ...
            fz_2d = np.zeros((endYear+1-startYear,latIndex[1]-latIndex[0]+1),np.float32)
            for ayear in range(startYear,endYear+1):
                ...
                for ind in range(latIndex[0],latIndex[1]+1):
                    fz_2d[ayear-startYear,ind-latIndex[0]] = Fz[prsInd,ind]
            
            # yearMeanFz[dnum-1] = np.nanmean(fz_1d)
            yearMeanFz[dnum-1] = np.nansum(fz_2d*w_lat)/np.sum(w_lat[~np.isnan(fz_2d)])
            print(f'{amonth}/{aday}')
...
def draw():
    ...
    axes.set_title(f'Fz intensity  prs={meanprs}hPa  latWeightMean {meanlatrange[0]}to{meanlatrange[1]}',fontsize=25)
    plt.grid(True)
    plt.savefig(f'D:/picture/study/MLS/Fz_intensity/movingMean_latWeightMean/prs{meanprs}_lat{meanlatrange[0]}to{meanlatrange[1]}Mean_E-Pflux_Fz_intensity.png')
    plt.show()
